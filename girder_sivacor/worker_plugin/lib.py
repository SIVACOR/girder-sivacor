    ...
    with tempfile.TemporaryDirectory() as container_temp_path:
        ...
        for stdout, stderr, key in [
            ...
            if key != "dockerstats":
                ...
                logging.info(f"Dumped {key} to {target_file}")
            ...
            with open(target_file, "rb") as fp:
                logging.info(
                    f"Reading {key} from {target_file} and uploading to Girder..."
                )
    ...
    if not task.canceled:
        if ret["StatusCode"] != 0:
            ...
            if not os.path.isfile(log_files["stdout"]):
                logging.error(
                    f"Expected Stata log file not found. {log_files.get('stdout')}"
                )
