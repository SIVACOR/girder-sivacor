"""Submission status tracking and outgoing email.

This used to live in ``worker_plugin`` and was bound to ``jobs.job.update.after``
inside the celery worker, which only worked because the worker shared Girder's
database. Workers now update jobs over REST, so the event fires on the server
and this has to live here.

Cleaning up a failed run's scratch directories stayed behind on the worker --
that is the one part of the old handler that has to happen where the files are.
"""
